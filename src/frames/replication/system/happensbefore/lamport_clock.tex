...
\begin{frame}{Horloge logique de Lamport}
  ...
  \begin{algorithm}[H]
    ...
    \When{dating an internal step $a$ \At $p_i$}{
      \nl $\TS_i \leftarrow \TS_i + 1$ \tcp*[r]{$\Time(a) = \TS_i$}
    }
    \When{$p_i$ sends a message $m$ \To $p_j$}{
      \nl $\TS_i \leftarrow \TS_i + 1$\\
      \nl \Send $\alert{\Stamped(m, \TS_i)}$ \To $p_j$;\tcp*[r]{$\Time(s_m) = \TS_i$}
    }
    \Upon{\Receive $\alert{\Stamped(m, t)}$ \From $p_j$ \At $p_i$}{
      \nl $\TS_i \leftarrow \max(\TS_i, t) + 1$\\
      \nl \Trigger \Receive $m$ \tcp*[r]{$\Time(r_m) = \TS_i > t$}
    }
  \end{algorithm}

  \begin{exampleblock}{Exemple d'exécution \only<2>{: zones temporelles}\onlyb<3>{: exécution indistinguable}}
    \only<-2>{
