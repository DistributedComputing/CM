...
\begin{frame}{La relation \og happened before \fg (ordre causal)}

  \begin{block}{Définition -- Happened before}

    \begin{itemize}
    \item $\hb$ est la plus petite relation telle que, pour tous pas $a$ et $a'$, \structure{$a\hb a'$} si:
      \begin{itemize}
    ...
    \end{itemize}
    \alert{Attention :} $\hb$ est seulement un ordre \alert{partiel} strict !
  \end{block}
  ...
  \begin{exampleblock}{Programme d'exemple : trois exécutions causales}
    ...
    \begin{tikzpicture}[y=10mm, anchor=center, x=36mm]
      ...
      \node at (2,1) {
        $\alert{s_1} || \example{s_2} \quad \land \quad \alert{r_1} || \example{r_2}$
      };
