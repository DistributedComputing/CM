...
\begin{frame}{Un deuxième exemple}
  ...
  \on[top, left=.5\textwidth]{
    \begin{algorithm}[H]
      \lProcess{$p_1$}{\Alert{\Send $\MSG_1$ \To $p_2$}}
    \end{algorithm}
  }
  \on[top, right=.5\textwidth]{
    \begin{algorithm}[H]
      \lProcess{$p_2$}{\Example{\Send $\MSG_2$ \To $p_1$}}
    \end{algorithm}
  }
  ...
  \on<2->[y=10mm]{
    \begin{tikzpicture}[automaton, x=30mm, y=12mm]
      ...
      \path[alert]   (11) edge node[left] {$p_1$ : \Send $\MSG_1$}    (12);
      \path[alert]   (12) edge node[left] {$p_2$ : \Receive $\MSG_1$} (13);
      \path[alert]   (21) edge node[left] {$p_1$ : \Send $\MSG_1$}    (22);
      \path[alert]   (22) edge node[right]{$p_2$ : \Receive $\MSG_1$} (23);
      \path[alert]   (31) edge node[right]{$p_1$ : \Send $\MSG_1$}    (32);
      \path[alert]   (32) edge node[right]{$p_2$ : \Receive $\MSG_1$} (33);
      \path[example] (11) edge node[above]{$p_2$ : \Send $\MSG_2$}    (21);
      \path[example] (21) edge node[above]{$p_1$ : \Receive $\MSG_2$} (31);
      \path[example] (12) edge node[below]{$p_2$ : \Send $\MSG_2$}    (22);
      \path[example] (22) edge node[above]{$p_1$ : \Receive $\MSG_2$} (32);
      \path[example] (13) edge node[below]{$p_2$ : \Send $\MSG_2$}    (23);
      \path[example] (23) edge node[below]{$p_1$ : \Receive $\MSG_2$} (33);
    \end{tikzpicture}
  }

  \onBlock<3->[bottom]{6 exécutions concrètes possibles (sans crash)}{
