...
\begin{frame}{Machine à états répliquée}
  ...
  \vspace{-1mm}
  \begin{block}{Interface -- Machine à états répliquée}
  ...
  \begin{tikzpicture}[y=8mm, x=9mm]
    ...
    \begin{scope}[alert]
      \node[fade on=<2>, operation] (o2) at (7.5,3) {\scriptsize\hspace{7mm}$\Consulter() \rightarrow \{b\}$\hspace{7mm}};
      \node[             operation] (s2) at (7.5,2) {\scriptsize$\Apply(\Consulter()) \rightarrow \{b\}$};
      \draw[fade on=<2>, message]   (o2.west) -- (s2.west);
      \draw[fade on=<2>, message]   (s2.east) -- (o2.east);
    \end{scope}

    \begin{scope}[example]
      \node[fade on=<2>, operation] (o3) at (5,0) {\scriptsize\hspace{7mm}$\Acheter(a) \rightarrow \cmark$\hspace{7mm}};
      \node[             operation] (s3) at (5,1) {\scriptsize$\Apply(\Acheter(a)) \rightarrow \cmark$};
      \draw[fade on=<2>, message]   (o3.west) -- (s3.west);
      \draw[fade on=<2>, message]   (s3.east) -- (o3.east);
    \end{scope}
  \end{tikzpicture}
  \end{exampleblock}

  \footnoteref{L. Lamport. \textit{The Implementation of Reliable Distributed Multiprocess Systems.} Computer Networks (1978)}
  \footnoteref{F. Schneider. \textit{Implementing fault-tolerant services using the state machine approach: A tutorial.} ACM CSUR (1990)}
