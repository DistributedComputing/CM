% SPDX-License-Identifier: CC-BY-SA-4.0
% Author: Matthieu Perrin

\begingroup

\begin{frame}{Différentes stratégies de réplication}

  \vspace{-3mm}
  \uncover{\hspace{\fill}\alert{Ce cours :}}
  \begin{itemize}
  \item \structure{Placement}  \hspace{\fill}\alert{pas de différence algorithmique}
    \begin{description}[mono-cloud :]
    \item[intra-site :] entre n\oe uds d'un même site
    \item[géo :] data centers de sites/régions différentes
    \end{description}

  \item<2->\vspace{1mm} \structure{Contrôle}  \hspace{\fill}\alert{pas de différence algorithmique}
    \begin{description}[mono-cloud :]
    \item[mono-cloud :] un fournisseur, même écosystème
    \item[multi-cloud :] plusieurs fournisseurs, interopérabilité à gérer
    \end{description}

  \item<3->\vspace{1mm} \structure{Mode} \hspace{\fill}\alert{réplication active car plus sûre}
    \begin{description}[mono-cloud :]
    \item[active :] toutes les répliques traitent des requêtes
    \item[passive :] un \structure{primaire} traite, des \structure{secondaires} recopient
    \end{description}

  \item<4->\vspace{1mm} \structure{Couverture} \hspace{\fill}\alert{réplication totale car plus simple}
    \begin{description}[mono-cloud :]
    \item[totale :] chaque réplique stocke tout l’état
    \item[partielle :] données découpées en fragments (shards)
    \end{description}

  \item<5->\vspace{1mm} \structure{Cohérence}  \hspace{\fill}\alert{surtout forte en CM}
    \begin{description}[mono-cloud :]
    \item[forte :] sûreté équivalente à un serveur unique
    \item[faible :] convergence, vues temporaires divergentes
    \end{description}

  \item<6->\vspace{1mm} \structure{Tolérance aux fautes} \hspace{\fill}\alert{crash car plus simple}
    \begin{description}[mono-cloud :]
    \item[crash :] un processus s'arrête, ne répond plus
    \item[byzantin :] un processus agit arbitrairement (erreurs ou malveillance)
    \end{description}
  \end{itemize}

\end{frame}

\endgroup
\endinput
