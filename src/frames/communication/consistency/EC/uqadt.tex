...
\begin{frame}{Machine à états à requêtes/mises à jour}
  ...
  \item[Requête :] \alert{$c \in \mathit{Query}$} ne modifie pas l'état
    \hspace{\fill} $\alert{\forall q\in Q, \tau(q, c) = q}$\\
  \item[Mise à jour :] \alert{$c \in \mathit{Update}$} retourne $\bot$
    \hspace{\fill} $\alert{\exists \bot\in R, \forall q\in Q, \rho(q, c) = \bot}$\\
  \end{description}

  \vspace{2mm}
  On étend la notation aux exécutions $E$ d'une implémentation répliquée de $M$ :
  \alert{$\mathit{Query}(E) = \{ c\rightarrow r \in E \mid c \in \mathit{Query} \}$ \hspace{\fill} $\mathit{Update}(E) = \{ c\rightarrow r \in E \mid c \in \mathit{Update} \}$}
