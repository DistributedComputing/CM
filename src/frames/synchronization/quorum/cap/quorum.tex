...
\begin{frame}{Surmonter CAP : la notion de quorum}
  ...
  \onBlock[top=-4mm]{Définitions}{
    ...
    \item un \structure{système de quorums} est un ensemble $\mathcal{Q} \subseteq \mathcal{P}(\Pi)$ tel que :
      $$\alert{\forall Q_1, Q_2 \in \mathcal{Q},\quad Q_1 \cap Q_2 \neq \emptyset}$$ 
  ...
  \onBlock[bottom=-2mm]{Remarque}{
    \begin{itemize}
    \item\vspace{-2mm} Il existe un quorum majoritaire de \alert{corrects} sous l'hypothèse \alert{$t<\frac{n}{2}$}
    \end{itemize}
  }

  \on[y=-16mm]{
    \begin{tikzpicture}[x=10mm, y=6mm, anchor=center]
      ...
      \node[structure, operation] (xr)  at (4.5,0) {$x.\Read() \rightarrow 1$};
