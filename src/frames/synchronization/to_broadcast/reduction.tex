...
\begin{frame}{Du consensus à \Broadcast{to}}
  ...
  \on[top=-5mm]{\small
    \begin{algorithm}[H]
      ...
      \Operation{\Broadcast{to} $(m)$}{
        \alt<6->{
          \Structureb{\Broadcast{rb} $\textsc{msg}(m)$}\;
        }{
          \Structureb<2>{$\mathit{broadcast}_i \leftarrow \mathit{broadcast}_i \cdot \langle m, i \rangle$}\;
        }
        \Exampleb<2,5>{\Wait $\langle m, i \rangle \in \mathit{delivered}_i$}\;
      }
      \Task{}{
        \For{\Alertb<-3>{$k = 1, 2, 3, ...$}}{
          \Structureb<2,4>{\Wait $\mathit{broadcast}_i \setminus \mathit{delivered}_i \neq \varepsilon$}\;
          \Structureb<2>{\Let $\mathit{proposed} \leftarrow \left(\mathit{broadcast}_i \setminus \mathit{delivered}_i\right)[1]$}\;
          \Alertb<-3>{\Let $\langle m, j \rangle \leftarrow \textsc{cons}[k].\Propose(\mathit{proposed})$}\;
          \Alertb<-3>{\Deliver{to} $m$ \From $p_j$}\;
          \Exampleb<2>{$\mathit{delivered}_i \leftarrow \mathit{delivered}_i \cdot \langle m, j \rangle$}\;
        }
      }
      \uncover<6->{
        \When{\Structureb{\Deliver{rb} $\textsc{msg}(m)$ \From $p_j$}}{
  ...
  \onAlertBlock<-2>[y=-9mm, anchor=north]{Variables}{\vspace{-1mm}
    \begin{description}
    \item<2>[$\structure{\mathit{broadcast}_i[k]}$] \vspace{-1mm} $k^{\text{e}}$ message \Broadcast{to} par $p_i$
    \item[${\textsc{cons}[k]}$] \vspace{-1mm} accord sur le $k^{\text{e}}$ message à délivrer
    \item<2>[${\color{exampleColor}\mathit{delivered}_i[k]}$] \vspace{-1mm} $k^{\text{e}}$ message \Delivered{to} par $p_i$ (résultat de ${\textsc{cons}[k]}$)
    \end{description}
  }
  ...
  \obAlertBlock<3>[y=-9mm, anchor=north]{Correction : sûreté}{
    \begin{itemize}
    \item Pour tout $k$, tous les processus décident le même $\langle m, j\rangle$ pour $\textsc{cons}[k]$
    \item Pour tout $p_i$, le $k^{e}$ message \Delivered{to} par $p_i$ est le même
    \end{itemize}
  }

  \obAlertBlock<4->[y=-9mm, anchor=north]{Correction : vivacité}{
    \begin{itemize}
    \item Livraison par les processus non-émetteurs ? 
    \item<5-> Terminaison de \Broadcast{to} ?
    \end{itemize}
  ...
  \ob<6>[bottom=-2mm]{
    \begin{citing}
    \item[CT96] \scriptsize Chandra, Toueg. \emph{Unreliable failure detectors for reliable distributed systems.} JACM (1996)
    \end{citing}
  }
