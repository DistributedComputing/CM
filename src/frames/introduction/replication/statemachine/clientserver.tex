...
\begin{frame}{Modèle client-serveur}

  \onBlock[top=-4mm]{Cas d'usage --  Site de vente en ligne}{
  ...
  \onBlock<2->[y=-2mm]{Solution client-serveur}{
    \begin{itemize}
    \item \structure{Un serveur :} maintient l'état (ensemble d'articles)
    \item \structure{Plusieurs clients :} messages avec le serveur (requête / réponse)
    \item \structure{Ordre séquentiel unique :} traitement des messages par le serveur
    \end{itemize}
  }
